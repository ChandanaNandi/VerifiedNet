"""Optional Gate 17B operational experiment: the REAL preregistered, one-run
BOUNDARY-ALIGNED-OBJECTIVE experiment on registered evaluation corpus v3.

This is Gate 16B's experiment with EXACTLY ONE variable changed: the training
objective is the Gate 17A boundary-aligned objective (objpol-7e6428964eae2db8),
which assembles input + target + EOS with input-only masking (no "\n"
separator), instead of the Gate 10F separator-bearing objective. The v2
contract-aligned input, targets, source examples, model, tokenizer, budget,
prompt, parser, and the entire measurement stack are held identical.

DOUBLE-GATED: the ``integration`` marker AND ``VERIFIEDNET_RUN_GATE17B=1``
...
run and one checkpoint, the same 64 ordered sources as Gate 15/16B, a clean
...
GATE17B_V3_CORPUS_ID = "evalcorpus-8c932345efc3e6e6"
GATE17B_V3_CORPUS_DIGEST = "ecdig-e72927cc7d4b6fd0fa141462"
GATE17B_READINESS_ID = "ready-0b128bea7400a13f"
GATE17B_V2_INPUT_TEMPLATE_ID = "traintmpl-c0513ab53036ae9b"
GATE17B_V2_POLICY_ID = "trainpolicy-336332a846b0f791"
GATE17B_TARGET_TEMPLATE_ID = "traintgt-286e4ecdff06833e"
#: The ONLY changed variable — the boundary-aligned objective (Gate 17A).
GATE17B_OBJECTIVE_POLICY_ID = "objpol-7e6428964eae2db8"
GATE16B_OBJECTIVE_POLICY_ID = "objpol-e5f36da1a1292f3d"
GATE17B_PROMPT_ID = "prompt-93808d932655a347"
GATE17B_MODEL_IDENTIFIER = "Qwen/Qwen2.5-0.5B-Instruct"
GATE17B_MODEL_REVISION = "7ae557604adf67be50417f59c2c2f167def9a775"
GATE17B_MODEL_ARCHITECTURE = "Qwen2ForCausalLM"
#: The exact Gate 15/16B envelope — held byte-for-byte.
GATE17B_EXAMPLE_CAP = 64
GATE17B_EPOCHS = 2
GATE17B_MAX_STEPS = 64
GATE17B_MAX_TOTAL_TOKENS = 448
GATE17B_EFFECTIVE_BATCH = 2
GATE17B_LEARNING_RATE = "0.00002"
GATE17B_WARMUP_STEPS = 4
GATE17B_SEED = 15

_ENABLED = os.environ.get("VERIFIEDNET_RUN_GATE17B") == "1"
_MODEL_DIR = os.environ.get("VERIFIEDNET_LOCAL_MODEL_DIR", "")
_V3_ROOT = os.environ.get("VERIFIEDNET_GATE17B_V3_ROOT", "")
_OUT_ROOT = os.environ.get("VERIFIEDNET_GATE17B_OUTPUT_ROOT", "")
...
#: the Gate 15/16B runs, their checkpoints, the Gate 10F.1 checkpoint, etc.
_PRIOR_DIRS = os.environ.get("VERIFIEDNET_GATE17B_PRIOR_ARTIFACT_DIRS", "")


def _skip_unless_enabled() -> tuple[Path, Path, Path, Path]:
    if not _ENABLED:
        pytest.skip("VERIFIEDNET_RUN_GATE17B!=1")
    ...
    if not _V3_ROOT or not Path(_V3_ROOT).is_dir():
        pytest.skip("VERIFIEDNET_GATE17B_V3_ROOT not set / not a dir")
    if not _OUT_ROOT:
        pytest.skip("VERIFIEDNET_GATE17B_OUTPUT_ROOT is not set")
    ...
    corpus_dir = v3_root / "evaluation-corpora" / GATE17B_V3_CORPUS_ID
    prepared_dir = v3_root / "chain" / "prepared"
    readiness_dir = v3_root / "readiness-assessments" / GATE17B_READINESS_ID
...
def test_gate17b_boundary_aligned_experiment_end_to_end(monkeypatch) -> None:
    ...
    def _boom(*args: object, **kwargs: object) -> object:
        raise AssertionError("Gate 17B must not use the network")
    ...
    assert v3.manifest.evaluation_corpus_id == GATE17B_V3_CORPUS_ID
    assert v3.manifest.corpus_digest == GATE17B_V3_CORPUS_DIGEST
    ...
    assert readiness["assessment_id"] == GATE17B_READINESS_ID
    ...
    assert v2_input_template.input_template_id == GATE17B_V2_INPUT_TEMPLATE_ID
    training_policy = contract_aligned_training_policy(
        task_id=task.task_id, input_template=v2_input_template,
        target_template=target_template)
    assert training_policy.training_data_policy_id == GATE17B_V2_POLICY_ID
    assert training_policy.target_template_id == GATE17B_TARGET_TEMPLATE_ID
    ...
    capped = cap_training_corpus(full_corpus,
                                 max_example_count=GATE17B_EXAMPLE_CAP)
    assert len(capped.examples) == GATE17B_EXAMPLE_CAP
    ...
    for v2e, v1e in zip(capped.examples, v1_capped.examples, strict=True):
        ...
        assert v2e.input.text != v1e.input.text     # v2 input (Gate 16A)
    ...
    assert sum(count for _f, count in families) == GATE17B_EXAMPLE_CAP
    assert len(families) == 4
    assert len(topologies) >= 4 and groups >= GATE17B_EXAMPLE_CAP // 4

    # ---- 3. the preregistered training configuration (Gate 15/16B exact) --
    model_fields = dict(provider="huggingface",
                        model_identifier=GATE17B_MODEL_IDENTIFIER,
                        model_revision=GATE17B_MODEL_REVISION,
                        model_class=GATE17B_MODEL_ARCHITECTURE)
    model_spec = TrainableModelSpec(
        **model_fields,
        model_spec_id=derive_model_spec_id(load_precision="float32",
                                           **model_fields))
    tok_fields = dict(tokenizer_identifier=GATE17B_MODEL_IDENTIFIER,
                      tokenizer_revision=GATE17B_MODEL_REVISION,
    ...
    spec = build_training_spec(
        ...
        sequence_policy=SequenceLengthPolicy(
            ...
            max_total_tokens=GATE17B_MAX_TOTAL_TOKENS),
        batch=BatchConfig(per_device_batch_size=1,
                          gradient_accumulation_steps=2,
                          effective_batch_size=GATE17B_EFFECTIVE_BATCH),
        optimization=OptimizationConfig(
            optimizer_name="adamw", learning_rate=GATE17B_LEARNING_RATE),
        scheduler=SchedulerConfig(scheduler_name="linear_warmup",
                                  warmup_steps=GATE17B_WARMUP_STEPS),
        budget=EpochBudget(epochs=GATE17B_EPOCHS),
        seed_policy=SeedPolicy(
            data_order_seed=GATE17B_SEED, model_init_seed=GATE17B_SEED,
            dropout_seed=GATE17B_SEED, backend_seed=GATE17B_SEED))
    plan = plan_for_real_backend(
        spec=spec, corpus=descriptor_from_manifest(corpus_manifest))
    assert plan.optimizer_steps <= GATE17B_MAX_STEPS
    ...
    envelope = ExperimentRuntimeEnvelope(
        max_examples=GATE17B_EXAMPLE_CAP, max_epochs=GATE17B_EPOCHS,
        max_optimizer_steps=GATE17B_MAX_STEPS,
        max_sequence_length=GATE17B_MAX_TOTAL_TOKENS,
        max_effective_batch_size=GATE17B_EFFECTIVE_BATCH)
    success_policy = build_success_policy(min_eligible_test_examples=30)
    objective_policy = boundary_aligned_objective_policy()
    assert objective_policy.objective_policy_id == GATE17B_OBJECTIVE_POLICY_ID
    assert objective_policy.objective_policy_id != GATE16B_OBJECTIVE_POLICY_ID
    assert objective_policy.sequence_construction == "input_target_eos"
    decoding = DecodingConfig(max_tokens=64)
    template = diagnosis_prompt_template()
    assert template.prompt_template_id == GATE17B_PROMPT_ID
    ...
    model_policy = build_bounded_model_policy(
        permitted_model_identifier=GATE17B_MODEL_IDENTIFIER,
        permitted_model_revision=GATE17B_MODEL_REVISION,
        permitted_architecture_class=GATE17B_MODEL_ARCHITECTURE,
        permitted_tokenizer_revision=GATE17B_MODEL_REVISION,
        max_declared_parameter_count=600_000_000,
        max_sequence_length=GATE17B_MAX_TOTAL_TOKENS,
        max_example_count=GATE17B_EXAMPLE_CAP, max_epochs=GATE17B_EPOCHS,
        max_optimizer_steps=GATE17B_MAX_STEPS,
        max_effective_batch_size=GATE17B_EFFECTIVE_BATCH)
    ...
    approval = build_model_approval(
        model_identifier=GATE17B_MODEL_IDENTIFIER,
        model_revision=GATE17B_MODEL_REVISION,
        tokenizer_identifier=GATE17B_MODEL_IDENTIFIER,
        tokenizer_revision=GATE17B_MODEL_REVISION,
        architecture_class=GATE17B_MODEL_ARCHITECTURE,
    ...
    experiment_spec = build_experiment_spec(
        ...
        scientific_question=(
            ...
            "examples, same v2 contract-aligned inputs, same targets, and same "
            "budget as Gate 16B, but with the boundary-aligned objective "
            "(input+target+EOS, input-only masking, no separator) so the first "
            "target token is supervised under the exact raw deployed prompt, "
            "improve structured-output validity and task performance?"),
        hypothesis=(
            "Removing the masked newline separator so the supervised "
            "first-target-token context is byte-identical to the raw deployed "
            "inference prefix will increase valid structured output relative to "
            "the matched base model and the Gate 16B empty-output treatment, and "
            "may improve accepted diagnosis; the null is no improvement in "
            "validity or accepted diagnosis performance."),
        evaluation_corpus_id=GATE17B_V3_CORPUS_ID,
        evaluation_corpus_digest=GATE17B_V3_CORPUS_DIGEST,
        readiness_assessment_id=GATE17B_READINESS_ID,
        ...
        training_example_cap=GATE17B_EXAMPLE_CAP,
        cap_rationale=(
            "identical to Gate 15/16B: the Literal-locked Gate 10F envelope "
            "permits at most 64 examples / 64 optimizer steps; deterministic "
            "first-64 canonical order — the SAME 64 sources as Gate 15/16B"),
    ...
    # ---- 5. authorization (fresh — NOT a prior run's) ---------------------
    ...
    slice_policy, slice_pairs = select_corpus_slice(
        written_corpus.root, max_example_count=GATE17B_EXAMPLE_CAP)
    assert len(slice_pairs) == GATE17B_EXAMPLE_CAP
    execution_policy = build_real_execution_policy(
        ...
        objective_policy_id=objective_policy.objective_policy_id,
        max_runtime_optimizer_steps=GATE17B_MAX_STEPS,
        max_epochs=GATE17B_EPOCHS, max_examples=GATE17B_EXAMPLE_CAP,
        max_sequence_length=GATE17B_MAX_TOTAL_TOKENS,
        max_effective_batch_size=GATE17B_EFFECTIVE_BATCH,
    ...
    assert loaded_exec.result.completed_optimizer_steps <= GATE17B_MAX_STEPS
    ...
    base_bundle = load_verified_base_model_bundle(
        base_dir, model_identifier=GATE17B_MODEL_IDENTIFIER,
        model_revision=GATE17B_MODEL_REVISION,
        architecture_class=GATE17B_MODEL_ARCHITECTURE,
    ...
    result = build_experiment_result(
        ...
        qualifiers=(
            ...
            "gate16b_treatment_valid_structured=0 (descriptive)"))
    ...
    print(f"GATE17B: base_valid={base_reliability.valid_structured_predictions}"
