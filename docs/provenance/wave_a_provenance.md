...
| G4-5 | `artifacts/{layout,writer,reader,verify}.py` | new (Gate 4) | low-level per-run artifact persistence: atomic canonical JSON + finalized JSONL, `.INCOMPLETE` marker, hash index + run digest, structured integrity verifier, offline reader; imports only schemas/common + pure ledger/transcript models (ADR 0016) | `tests/unit/test_artifacts_*`, `tests/contract/test_artifact_shapes.py`, `tests/property/test_artifact_hash_properties.py`, `tests/failure/test_artifact_failures.py`, `tests/security/test_artifact_boundaries.py` |
