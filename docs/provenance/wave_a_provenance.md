...
| 10 | `faults/frr_commands.py` | sonic-troubleshooting-agent (MIT) | eb4c818 | `_apply_inject`, `_apply_restore` vtysh sequences | REFACTOR_AND_REUSE / copy with modifications | re-targeted at plain FRR; ASN/peer as data; clear-bgp retained (load-bearing, annotated in records) | `tests/unit/test_faults_commands.py` |
